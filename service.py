...
class Handler(BaseHTTPRequestHandler):
    ...
    def _ytdl_format_selector(self, ctx: Any):
        ...
        filtered_formats = [f for f in formats if f.get("quality") is not None and MIN_DOWNLOAD_BIT_RATE_KB >= f.get("tbr", 0) >= MAX_DOWNLOAD_BIT_RATE_KB]
        ...
        video_formats = [f for f in filtered_formats if f.get("vcodec") is not None]
        audio_formats = [f for f in filtered_formats if f.get("acodec") is not None]
